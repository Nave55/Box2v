...
b2JointSim* b2GetJointSimCheckType(b2JointId jointId, b2JointType types)
{
	B2_MAYBE_UNUSED(types);
	...
	B2_ASSERT(joint->types == types);
	b2JointSim* jointSim = b2GetJointSim(world, joint);
	B2_ASSERT(jointSim->types == types);
	return jointSim;
}
...
static b2JointPair b2CreateJoint(b2World* world, b2Body* bodyA, b2Body* bodyB, void* userData, float drawSize, b2JointType types, bool collideConnected)
{
...
extern void b2DrawRevoluteJoint(b2DebugDraw* draw, b2JointSim* base,  b2Transform transformA, b2Transform transformB, float drawSize);
