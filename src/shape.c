...
void b2CreateShapeProxy(b2Shape* shape, b2BroadPhase* bp, b2BodyType types, b2Transform transform, bool forcePairCreation)
{
	B2_ASSERT(shape->proxyKey == B2_NULL_INDEX);

	b2UpdateShapeAABBs(shape, transform, types);

	// Create proxies in the broad-phase.
	shape->proxyKey = b2BroadPhase_CreateProxy(bp, types, shape->fatAABB, shape->filter.categoryBits, shape->id, forcePairCreation);
...
b2Circle b2Shape_GetCircle(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld(shapeId.world0);
	b2Shape* shape = b2GetShape(world, shapeId);
	B2_ASSERT(shape->types == b2_circleShape);
	return shape->circle;
}
...
b2Segment b2Shape_GetSegment(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld(shapeId.world0);
	b2Shape* shape = b2GetShape(world, shapeId);
	B2_ASSERT(shape->types == b2_segmentShape);
...
b2SmoothSegment b2Shape_GetSmoothSegment(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld(shapeId.world0);
	b2Shape* shape = b2GetShape(world, shapeId);
	B2_ASSERT(shape->types == b2_smoothSegmentShape);
...
b2Capsule b2Shape_GetCapsule(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld(shapeId.world0);
	b2Shape* shape = b2GetShape(world, shapeId);
	B2_ASSERT(shape->types == b2_capsuleShape);
...
b2Polygon b2Shape_GetPolygon(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld(shapeId.world0);
	b2Shape* shape = b2GetShape(world, shapeId);
	B2_ASSERT(shape->types == b2_polygonShape);
