...
b2BodyId b2CreateBody(b2WorldId worldId, const b2BodyDef* def)
{
	...
	{
		// any body types can be disabled
		setId = b2_disabledSet;
	}
	else if (def->types == b2_staticBody)
	{
		setId = b2_staticSet;
	}
...
// Changing the body types is quite complex mainly due to joints.
// Considerations:
...
void b2Body_SetType(b2BodyId bodyId, b2BodyType types)
{
	b2World* world = b2GetWorld(bodyId.world0);
	b2Body* body = b2GetBodyFullId(world, bodyId);

	b2BodyType originalType = body->types;
	if (originalType == types)
	{
		return;
	}
	...
	{
		// Disabled bodies don't change solver sets or islands when they change types.
		body->types = types;

		// Body types affects the mass
	...
	else
	{
		...
		B2_ASSERT(types == b2_dynamicBody || types == b2_kinematicBody);
	...
	// Relink all joints
	{
		...
		while (jointKey != B2_NULL_INDEX)
		{
			...
			if (body->types == b2_staticBody && otherBody->types == b2_staticBody)
			{
				continue;
			}
...
void b2Body_Enable(b2BodyId bodyId)
{
	...
	b2SolverSet* disabledSet = world->solverSetArray + b2_disabledSet;
	int setId = body->types == b2_staticBody ? b2_staticSet : b2_awakeSet;
	...
	b2BodyType proxyType = body->types;
	bool forcePairCreation = true;
	int shapeId = body->headShapeId;
	while (shapeId != B2_NULL_INDEX)
	{
...
{
	if (bodyA->types != b2_dynamicBody && bodyB->types != b2_dynamicBody)
	{
		return false;
	}
