...
static bool DrawQueryCallback(int proxyId, int shapeId, void* context)
{
	...
	if (draw->drawShapes)
	{
		...
		if (body->types == b2_dynamicBody && bodySim->mass == 0.0f)
		{
			// Bad body
			color = b2_colorRed;
		}
		...
		else if (body->types == b2_staticBody)
		{
			color = b2_colorPaleGreen;
		}
		else if (body->types == b2_kinematicBody)
		{
...
static void b2DrawWithBounds(b2World* world, b2DebugDraw* draw)
{
	...
	for (uint32_t k = 0; k < wordCount; ++k)
	{
		...
		while (word != 0)
		{
			...
			if (draw->drawMass && body->types == b2_dynamicBody)
			{
			...
			if (draw->drawContacts && body->types == b2_dynamicBody && body->setIndex == b2_awakeSet)
			{
		...
		b2GraphColor* color = world->constraintGraph.colors + colorIndex;
		{
			...
			for (int i = 0; i < color->contacts.count; ++i)
			{
				...
				if (colorIndex < b2_overflowIndex)
				{
					...
					B2_ASSERT(b2GetBit(&color->bodySet, bodyIdA) == (bodyA->types != b2_staticBody));
					B2_ASSERT(b2GetBit(&color->bodySet, bodyIdB) == (bodyB->types != b2_staticBody));
				}
			}
		}
