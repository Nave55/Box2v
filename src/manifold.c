											...
											b2Transform xfB, b2DistanceCache* cache)
{
	...
	if (behind1 == false && output.distance > 0.1f * b2_linearSlop)
	{
		...
		if (cache->count == 1)
		{
			...
			enum b2NormalType types = b2ClassifyNormal(smoothParams, normal);
			if (types == b2_normalSkip)
			{
				return manifold;
			}
			
			if (types == b2_normalAdmit)
			{
				manifold.normal = b2RotateVector(xfA.q, normal);
				b2ManifoldPoint* cp = manifold.points + 0;
		...
		else
		{
			...
			if (ia1 == ia2)
			{
				...
				enum b2NormalType types = b2ClassifyNormal(smoothParams, b2Neg(normalB));
				if (types == b2_normalSkip)
				{
					return manifold;
				}
	...
	else
	{
		...
		for (int32_t i = 0; i < count; ++i)
		{
			...
			enum b2NormalType types = b2ClassifyNormal(smoothParams, b2Neg(n));
			if (types != b2_normalAdmit)
			{
				continue;
			}
