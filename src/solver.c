...
static void b2FinalizeBodiesTask(int startIndex, int endIndex, uint32_t threadIndex, void* context)
{
	...
	for (int simIndex = startIndex; simIndex < endIndex; ++simIndex)
	{
		...
		if (enableSleep == false || body->enableSleep == false || sleepVelocity > body->sleepThreshold)
		{
			...
			if (body->types == b2_dynamicBody && enableContinuous && maxVelocity * timeStep > saftetyFactor * sim->minExtent)
			{
...
{
	b2SolverStageType stageType = stage->types;
...
static void b2ExecuteStage(b2SolverStage* stage, b2StepContext* context, int previousSyncIndex, int syncIndex, int workerIndex)
{
	...
	{
		B2_ASSERT(stage->types != b2_stagePrepareContacts || syncIndex < 2);
...
void b2SolverTask(int startIndex, int endIndex, uint32_t threadIndexDontUse, void* taskContext)
{
	...
	if (workerIndex == 0)
	{
		...
		*/
		...
		B2_ASSERT(stages[stageIndex].types == b2_stagePrepareJoints);
		...
		B2_ASSERT(stages[stageIndex].types == b2_stagePrepareContacts);
		b2ExecuteMainStage(stages + stageIndex, context, syncBits);
		stageIndex += 1;
		contactSyncIndex += 1;
		...
		for (int i = 0; i < subStepCount; ++i)
		{
			...
			B2_ASSERT(stages[iterStageIndex].types == b2_stageIntegrateVelocities);
			...
			for (int colorIndex = 0; colorIndex < activeColorCount; ++colorIndex)
			{
				...
				B2_ASSERT(stages[iterStageIndex].types == b2_stageWarmStart);
				b2ExecuteMainStage(stages + iterStageIndex, context, syncBits);
				iterStageIndex += 1;
			}
			graphSyncIndex += 1;
			...
			{
				syncBits = (graphSyncIndex << 16) | iterStageIndex;
				B2_ASSERT(stages[iterStageIndex].types == b2_stageSolve);
				b2ExecuteMainStage(stages + iterStageIndex, context, syncBits);
				iterStageIndex += 1;
			}
			graphSyncIndex += 1;
			...
			// integrate positions
			B2_ASSERT(stages[iterStageIndex].types == b2_stageIntegratePositions);
			syncBits = (bodySyncIndex << 16) | iterStageIndex;
			b2ExecuteMainStage(stages + iterStageIndex, context, syncBits);
			iterStageIndex += 1;
			bodySyncIndex += 1;
			...
			{
				syncBits = (graphSyncIndex << 16) | iterStageIndex;
				B2_ASSERT(stages[iterStageIndex].types == b2_stageRelax);
				b2ExecuteMainStage(stages + iterStageIndex, context, syncBits);
				iterStageIndex += 1;
			}
			graphSyncIndex += 1;
		...
		// Restitution
		{
			...
			{
				syncBits = (graphSyncIndex << 16) | iterStageIndex;
				B2_ASSERT(stages[iterStageIndex].types == b2_stageRestitution);
				b2ExecuteMainStage(stages + iterStageIndex, context, syncBits);
				iterStageIndex += 1;
			}
			// graphSyncIndex += 1;
		...
		syncBits = (contactSyncIndex << 16) | stageIndex;
		B2_ASSERT(stages[stageIndex].types == b2_stageStoreImpulses);
		b2ExecuteMainStage(stages + stageIndex, context, syncBits);
...
static bool b2ContinuousQueryCallback(int proxyId, int shapeId, void* context)
{
	...
	B2_ASSERT(body->types == b2_staticBody || fastBodySim->isBullet);
	...
	if (shape->types == b2_smoothSegmentShape)
	{
...
void b2Solve(b2World* world, b2StepContext* stepContext)
{
	...
	// Solve constraints using graph coloring
	{
		...
		// Prepare joints
		stage->types = b2_stagePrepareJoints;
		...
		// Prepare contacts
		stage->types = b2_stagePrepareContacts;
		...
		// Integrate velocities
		stage->types = b2_stageIntegrateVelocities;
		...
		// Integrate positions
		stage->types = b2_stageIntegratePositions;
		stage->blocks = bodyBlocks;
		stage->blockCount = bodyBlockCount;
		stage->colorIndex = -1;
		stage->completionCount = 0;
		...
		for (int i = 0; i < activeColorCount; ++i)
		{
			stage->types = b2_stageRestitution;
			stage->blocks = graphColorBlocks[i];
			stage->blockCount = colorJointBlockCounts[i] + colorContactBlockCounts[i];
			stage->colorIndex = activeColorIndices[i];
			stage->completionCount = 0;
		...
		// Store impulses
		stage->types = b2_stageStoreImpulses;
		stage->blocks = contactBlocks;
		stage->blockCount = contactBlockCount;
		stage->colorIndex = -1;
		stage->completionCount = 0;
